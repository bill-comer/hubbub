                    ...
                    <table>
                        ...
                        <tr class="prop">
                            ...
                            <g:if test="${userInstance?.profile?.photo}">
                              <img src="<g:createLink controller='user' action='profile' id='${fieldValue(bean: userInstance, field: "userId")}'/>"/>
                            </g:if>
                            </td>
                            ...
                            <tr class="prop">
                                <td valign="top" class="name">
                                  <label for="following"><g:message code="user.following.label" default="Following" /></label>
                                </td>
                                <td valign="top" class="value ${hasErrors(bean: userInstance, field: 'following', 'errors')}">
                                    <g:select name="following" from="${com.grailsinaction.User.list()}" multiple="yes" optionKey="id" size="5" value="${userInstance?.following*.id}" />
                                </td>
                            </tr>
...
<g:link controller="post" action="create" params="['user.id': userInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'post.label', default: 'Post')])}</g:link>
                            ...
                            <tr class="prop">
                                <td valign="top" class="name">
                                  <label for="tags"><g:message code="user.tags.label" default="Tags" /></label>
                                </td>
...
<g:link controller="tag" action="create" params="['user.id': userInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'tag.label', default: 'Tag')])}</g:link>
                ...
                <div class="buttons">
                    ...
                    <g:link controller="image" action="upload" params="['id': userInstance?.id]">Upload Photo</g:link>
                </div>
            </g:form>
        </div>
    </body>
